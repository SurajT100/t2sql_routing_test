...
class ContextAgent:
    ...
    def fetch_context(
        ...
        # Safety fallback:
        # If table-focused filtering removed everything but Stage 2 had rules,
        # keep the original retrieved rules so reasoning still has business guidance.
        if bundle.rules_retrieved == 0 and source_count > 0:
            bundle.rules_compressed = rules_compressed
            bundle.rules_retrieved = source_count
            print("[CONTEXT AGENT] Rule fallback: retained original Stage 2 rules "
                  "because focused filtering produced zero rules")
